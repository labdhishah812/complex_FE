...
// const RolesCreate = ({ editData, onHide }) => {
...
//     console.log(rolesDataById, ':::rolesDataById');
//     useEffect(() => {
//         if (params.id) {
...
//                                         // onClick={onHide}
//                                         onClick={() => navigate('/property-management/roles')}
//                                     />
...
const RolesCreate = ({ editData, onHide }) => {
    ...
    const onCheck = (index, name) => {
        try {
            ...
            const allChecked = array.every((item) => {
                if (item.module_name === 'dashboard' || (item.module_name === 'rental' && (name === 'create' || name === 'update' || name === 'delete'))) {
                    return true;
                }
    ...
    const onHeaderCheck = (permission) => {
        ...
        const updatedRolesArray = rolesArray.map((role) => {
            if (role.module_name === 'dashboard' || (role.module_name === 'rental' && (permission === 'create' || permission === 'update' || permission === 'delete'))) {
                return role;
            }
                        ...
                        <Form>
                            <div className="grid p-fluid mt-1">
                                <div className="field col-12 md:col-4 mb-1">
                                    ...
                                    <InputText id="roleName" name="roleName" placeholder="Enter Role Name" type="text" value={values.roleName} onChange={handleChange} className={classNames({ 'p-invalid': errors.roleName && touched.roleName })} />
                                    <div className="p-invalid error text-xs" style={{ minHeight: '1.1rem', marginTop: '3px' }}>
                                        {errors.roleName && touched.roleName ? errors.roleName : ''}
                                    </div>
                                </div>
                                        ...
                                        <tbody>
                                            {rolesArray.map((e, i) => (
                                                <tr key={i}>
                                                    <td className="roles_table_td">{e.roleName}</td>
                                                    <td className="roles_table_td">
                                                        <Checkbox onChange={() => onCheck(i, 'create')} checked={e.roles.findIndex((x) => x === 'create') !== -1} disabled={e.module_name === 'dashboard' || e.module_name === 'rental'} />
                                                    </td>
                                                    <td className="roles_table_td">
                                                        <Checkbox onChange={() => onCheck(i, 'read')} checked={e.roles.findIndex((x) => x === 'read') !== -1} disabled={e.module_name === 'dashboard'} />
                                                    </td>
                                                    <td className="roles_table_td">
                                                        <Checkbox onChange={() => onCheck(i, 'update')} checked={e.roles.findIndex((x) => x === 'update') !== -1} disabled={e.module_name === 'dashboard' || e.module_name === 'rental'} />
                                                    </td>
                                                    <td className="roles_table_td">
                                                        <Checkbox onChange={() => onCheck(i, 'delete')} checked={e.roles.findIndex((x) => x === 'delete') !== -1} disabled={e.module_name === 'dashboard' || e.module_name === 'rental'} />
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                            ...
                            <div className="grid p-fluid mt-1">
                                <div className="field col-12 md:col-12 mb-1 flex justify-content-end">
                                    <Button label="Cancel" icon="pi pi-times" className="p-button-outlined p-button-danger mr-2 w-7rem" onClick={() => navigate('/property-management/roles')} />
                                    <Button disabled={submitted} label={'Submit'} type="submit" icon="pi pi-check" className="p-button-outlined p-button-success mr-2 w-7rem" />
                                </div>
                            </div>
                        </Form>
