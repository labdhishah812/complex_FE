...
import { getCommitteeData, committeeRemoveRequest, getGeneralMemberListData } from '../../../../redux/slice/AdminSlices/committeeSlice';
import moment from 'moment-timezone';
import axios from 'axios';
import ExcelJS from 'exceljs';
const GeneralCoreMembers = () => {
    ...
    const { isCreated, isDelete, generalListData, isLoading } = useSelector((state) => state.committee);
    ...
    const callCommitteeList = (val) => {
        try {
            dispatch(getGeneralMemberListData(val));
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const rightToolbarTemplate = () => {
        return (
            <React.Fragment>
                <Button
                    disabled={generalListData?.data?.role_listing.length === 0}
                    label="Export"
                    icon="pi pi-download"
                    className="p-button-outlined p-button-help mr-2 my-3 m-auto"
                    onClick={async () => {
                        try {
                            const { data } = await axios.post(`${BASE_URL_API}/user/generalCommitteExcel`, pagination, {
                                headers: {
                            ...
                            const memberData = generalListData?.data?.role_listing; // Use existing data instead of API response

                            if (memberData && memberData.length > 0) {
                                const workbook = new ExcelJS.Workbook();
                                const worksheet = workbook.addWorksheet('General Core Members');
                                ...
                                a.href = url;
                                a.download = 'general-core-members.xlsx';
                                a.click();
                ...
                <Button
                    ...
                    onClick={() => navigate(`/property-management/general-core-members/general-core-members-add`)}
                />
            </React.Fragment>
        );
    };
    ...
    const leftToolbarTemplate = () => {
        const isSearchDisabled = generalListData?.data?.role_listing.length === 0 && pagination?.search === '';
        return (
            <React.Fragment>
        ...
        return (
            <div className="actions flex justify-content-center">
                <Button
                    ...
                    onClick={() => navigate(`/property-management/general-core-members/general-core-members-edit/${rowData?._id}`)}
                />
        ...
        <div className="relative min-h-full">
            ...
            <div className="flex justify-content-between align-items-center">
                <div className="flex flex-row w-full">
                    <h5 className="title m-2 pr-3 flex align-items-center justify-content-center">{'General Core Members'}</h5>
                    <BreadCrumb model={breadcrumbItems} home={breadcrumbHome} className="layout-breadcrumb p-pl-3 p-py-2 ml-auto" />
                </div>
            </div>
            <div className="grid crud-demo ml-0 mr-0 mt-3">
                <div className="col-12 card">
                    {generalListData?.data?.role_listing && generalListData?.data?.role_listing.length > 0 ? (
                        <DataTable
                            value={generalListData?.data?.role_listing}
                            showGridlines
                            stripedRows
                            dataKey="id"
                        ...
                        >
                            ...
                            <Column
                                ...
                                body={(rowData) => (
                                    <a
                                        href={`${BASE_URL_API}property-management/general-core-members/${rowData.user_property_assign_id}`}
                                        className=" hover:underline"
                                        onClick={(e) => {
                                            e.preventDefault(); // Prevent default navigation
                                            navigate(`/property-management/general-core-members/${rowData.user_property_assign_id}`);
                            ...
                            <Column
                                ...
                                body={(rowData) => (
                                    <div className="text-right">
                                        <a href={`tel:${rowData?.mobile_number}`} className=" hover:underline">
                        ...
                        <DataTable
                            emptyMessage={() => (
                                <>
                                    <div className="flex-wrap flex">{generalListData?.data?.role_listing && generalListData?.data?.role_listing.length === 0 && <img src={paper} className="h-20rem w-20rem m-auto" />}</div>
                                    {generalListData?.data?.role_listing && generalListData?.data?.role_listing.length === 0 && <div className="text-center text-2xl">{'No Record Found.'}</div>}
                                </>
                            )}
                            value={[]}
                            header={header}
...
export default GeneralCoreMembers;
