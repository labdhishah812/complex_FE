...
const Profile = () => {
    const { useNavigate, Image, BreadCrumb, Dropdown, Password, TabPanel, TabView, classNames, InputText, useEffect, Button, React, useDispatch, useSelector, Link, useState } = components;
    ...
    useEffect(() => {
        setActiveIndex(0); // Reset to Profile tab on component mount
    }, []);
                                                ...
                                                {loginDetails?.role !== 'Super Admin' && (
                                                    <div className="field col-12 md:col-4 mb-1">
                                                        ...
                                                        <Dropdown
                                                            ...
                                                            value={values?.gender}
                                                            onChange={(e) => {
                                                                setFieldValue('gender', e.value); // Fix: Use e.value for Dropdown value
                                                                setDisabledBTN(false); // Enable button or other actions as needed
                                                            }}
                                                            className={classNames({ 'p-invalid': errors.gender && touched.gender })}
                                                        />
                                                        {errors.gender && touched.gender && <small className="p-invalid error">{errors.gender}</small>}
                                                    </div>
                                                )}
                                                <div className="field col-12 md:col-4 mb-1">
                                                    <label htmlFor="email">
                                                    ...
                                                    <InputText id="email" name="email" disabled placeholder="Enter email" type="text" value={values?.email} onChange={(e) => { setFieldValue("email", e.target.value); setDisabledBTN(false); }} className={classNames({ 'p-invalid': errors.email && touched.email })} />
