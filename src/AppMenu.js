...
const AppSubmenu = forwardRef((props, ref) => {
    ...
    const onMenuItemClick = (event, item, index) => {
        ...
        if (item.items === undefined && activeIndex !== null) {
            setActiveIndex(null);
        }
    ...
    const getLink = (item, index) => {
        ...
        const content = (
            <>
                ...
                {item.icon ? item.icon : ""}
                <span className={`layout-menuitem-text ${!item.icon && "ml-3"}`}>{item.label}</span>
        ...
        return (
            <NavLink data-pr-tooltip={props.root && item.label} to={item.to} {...commonLinkProps} className={({ isActive }) => classNames(commonLinkProps.className, isActive ? 'router-link-active' : undefined)}>
                {content}
            </NavLink>
        );
    };
                ...
                return (
                    <li key={item.label || i} className={styleClass} role="menuitem">
                        ...
                        <CSSTransition
                            ...
                            in={item.items && (props.root && !((isHorizontal() || isSlim()) && !isMobile() && (!isSlim() || (isSlim() && activeIndex !== null))) ? true : active)}
                            unmountOnExit
                        >
...
const AppMenu = (props) => {
    return (
        <AppSubmenu items={props.model} className="layout-menu mb-auto" menuActive={props.active} onRootItemClick={props.onRootMenuItemClick} onMenuItemClick={props.onMenuItemClick} root={true} menuMode={props.menuMode} parentMenuItemActive={true} />
    );
};
